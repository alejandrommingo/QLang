"""
Finding and sampling units from a corpus.

This module does two separate jobs, kept apart on purpose:

  A) WHAT to observe (extractors): turn a corpus into a list of "units" --
     one per word occurrence (extract_segments), per sentence
     (extract_sentences), or per document (extract_documents). Every unit
     carries its doc_id, text, position (offset) and metadata.

  B) HOW to pick a sample (strategies): given any list of units, choose some
     of them. The strategies -- exhaustive (keep all), random_simple (n at
     random), stratified (n keeping group proportions), reservoir (n at random
     from a stream) -- work on any list without caring what the units are.

On top of these there are controls to keep a sample balanced (a cap per
document, per-group summaries) and a two-stage flow for word sampling that
ends in a KWIC concordance (each occurrence shown with its surrounding text):

  STAGE 1 (stage1_documents): keep the documents that contain the word,
    reduced to N of them.
  STAGE 2 (stage2_occurrences): find the word's occurrences inside those
    documents, cap how many per document, apply a strategy, and add the
    context window (the KWIC view).

Saving utilities (save_units) and the TraceLog keep every step traceable.
...
def extract_sentences(corpus: Corpus,
                      ...
                      use_spacy: bool = True,
                      spacy_model: str = "") -> List[Unit]:
    """One unit per sentence.

    Sentence splitting is delicate (abbreviations, line breaks, etc.). If
    'use_spacy' is True and spaCy is installed, a more robust splitter is used;
    otherwise it falls back to a simple rule (split on . ! ? + whitespace).
    The simple rule already handles line breaks, but not abbreviations.

    'spacy_model' lets you pick a model (e.g. 'es_core_news_sm' for Spanish,
    'en_core_web_sm' for English); empty uses a light blank pipeline with just
    a rule-based sentence boundary component. NOTE: the basic sentencizer is
    still rule-based and does NOT handle abbreviations (Dr., etc.) better than
    the fallback; to get that, pass a trained 'spacy_model' (which must be
    installed/downloaded). The main robust gains over the fallback come from a
    trained model.
    """
    cl.validate_corpus(corpus)
    splitter = _make_sentence_splitter(use_spacy, spacy_model)
    units: List[Unit] = []
    items = corpus.items()
    for doc_id, doc in _progress(items, total=len(corpus),
        ...
        body = doc["body"]
        meta = doc.get("meta", {})
        for start, end in splitter(body):
            raw = body[start:end]
            text = raw.strip()
            if not text:
                continue
...
def _make_sentence_splitter(use_spacy: bool, spacy_model: str):
    """Return a function body -> list of (start, end) sentence spans.

    Behaviour:
      - use_spacy False: the simple rule-based splitter.
      - use_spacy True, no model name: spaCy's light rule-based sentencizer
        (no download; not better with abbreviations than the fallback).
      - use_spacy True, a model name (e.g. 'es_core_news_sm'): loads that
        TRAINED model, downloading it automatically the first time if needed.
        Trained models split sentences much better (abbreviations, etc.).

    If anything fails (spaCy missing, download fails, no internet), it warns
    and falls back to the simple rule splitter, so the pipeline never breaks.
    Done once per extraction to avoid reloading spaCy.
    """
    if not use_spacy:
        return _sentence_bounds

    try:
        import spacy
    except ImportError:
        print("  [sentences] spaCy is not installed; using the simple "
              "splitter. Install with: pip install spacy")
        return _sentence_bounds

    try:
        if spacy_model:
            nlp = _load_or_download_spacy(spacy, spacy_model)
        else:
            # light pipeline: blank model + rule-based sentencizer, no download
            nlp = spacy.blank("xx")
            nlp.add_pipe("sentencizer")

        def split_spacy(body: str):
            doc = nlp(body)
            return [(s.start_char, s.end_char) for s in doc.sents]

        return split_spacy
    except Exception as e:
        print(f"  [sentences] could not set up spaCy ({e}); using the simple "
              f"splitter instead.")
        return _sentence_bounds


def _load_or_download_spacy(spacy, model_name: str):
    """Load a trained spaCy model, downloading it the first time if missing.

    Tries spacy.load; if the model isn't installed, runs 'spacy download' once
    and loads again. Raises if it still can't be loaded (the caller then falls
    back to the simple splitter).
    """
    try:
        return spacy.load(model_name)
    except OSError:
        # not installed yet: download it (needs internet, done once)
        print(f"  [sentences] downloading spaCy model '{model_name}' "
              f"(first time only)...")
...
def select_sentences_with_word(corpus: Corpus, word: str,
                               mode: str = "exact",
                               show_progress: bool = False,
                               use_spacy: bool = True,
                               spacy_model: str = "") -> List[Unit]:
    """Sentences that contain the word, as units.

    Like extract_sentences, but keeps only the sentences where the word
    appears (using the same match modes as the segment extractors). Useful
    when the unit of analysis is the sentence but you only care about the ones
    mentioning a target word.
    """
    pattern = build_pattern(word, mode)
    sentences = extract_sentences(corpus, show_progress=show_progress,
                                  use_spacy=use_spacy, spacy_model=spacy_model)
    return [u for u in sentences if pattern.search(u["text"])]
